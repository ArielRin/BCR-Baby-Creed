...
_FeeMutualFunds:    _Fee_FeeMutualFunds = value_[1];        0
Treasury:            _FeeTreasury = value_[2];              3
autoLP:             _FeeLiquidityPool = value_[3];          2
buybacks:           _FeeBuyback = value_[4];                0
reflection:         _FeeReflections = value_[5];            0

BBCR1 addresses used after supplied to @Dev
InvestmentStocks      0x87e8B18Fda7A61db217E1C279135566A90862479
FeeMutualFunds       0x7699c7f12c321eE818dF5066178A3E15EB27c498
(Team) Treasury    0x5a501DD7BF174cd1aE7a4DE5260Ec5717CbC7B19
IUniswapV2Router      020x10ED43C718714eb63d5aA57B78B54704E256024E
buybacks wallet       0x3A9D1bA4cA2713CF3226d62218CB77eF0028fc55

address deploy string per chain (change router)
// bsc          ["0x87e8B18Fda7A61db217E1C279135566A90862479","0x7699c7f12c321eE818dF5066178A3E15EB27c498","0x5a501DD7BF174cd1aE7a4DE5260Ec5717CbC7B19","0x10ED43C718714eb63d5aA57B78B54704E256024E","0x3A9D1bA4cA2713CF3226d62218CB77eF0028fc55"]
...
contract BabyCreed is ERC20, Ownable {
    ...
    uint256 public snipeBlockAmount;
    bool public snipeBlockEnabled = true;
    mapping (address => uint256) public lastTxTime;
    uint256 public txCooldown = 30 seconds; // Cooldown period between transactions
...
function _transfer(
    ...
    // Cooldown and snipe protection
    if (sender != owner() && recipient != owner()) {
        require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");

        // Snipe block protection
        if (snipeBlockEnabled && block.number < launchTime + snipeBlockAmount) {
            require(sender == owner(), "Transfer: Snipe protection active");
        }

        // Cooldown timer check
        require(lastTxTime[sender] + txCooldown <= block.timestamp, "Transfer: Cooldown period active");
    ...
    launchTime = time;
    snipeBlockAmount = 3; // Number of blocks to enable snipe protection
    }

    function setSnipeBlockEnabled(bool enabled) external onlyOwner {
    snipeBlockEnabled = enabled;
    }
    // @cheyne cooldown
    modifier transactionCooldown() {
    require(lastTxTime[msg.sender] + txCooldown <= block.timestamp, "Please wait for cooldown between transactions");
    _;
